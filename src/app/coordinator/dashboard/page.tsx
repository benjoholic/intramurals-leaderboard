...
export default function CoordinatorDashboard() {
  ...
  // Navigation items for coordinator
  const navItems = [
    { title: "Dashboard", icon: Home, url: "/coordinator/dashboard", isActive: true },
  ...
  // Simple UI placeholders used by admin page; keep minimal here
  ...
  const [recentActivities] = useState([
    ...
    { id: "a3", text: "Settings updated by Coordinator", time: "3d ago" },
  ])
                ...
                {stats.map((s) => (
                  <div key={s.id} className="bg-white rounded-2xl p-6 shadow">
                    <div className="flex items-center justify-between">
                      ...
                      {s.delta && <div className="text-sm text-green-600 font-medium">{s.delta}</div>}
                    </div>
                  </div>
                ))}
              </div>
      ...
      <AlertDialog open={showSignOutDialog} onOpenChange={setShowSignOutDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to sign out? You will need to log in again to access the coordinator panel.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
