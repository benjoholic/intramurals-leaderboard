...
import { UserPlus, Mail, Lock, Eye, EyeOff, ArrowRight, Trophy, AlertCircle, CheckCircle, User } from "lucide-react"
...
function FloatingPaths({ position }: { position: number }) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      ...
      >
        <defs>
          <linearGradient id="path-gradient-signup" x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        {paths.map((path) => (
          <motion.path
            key={path.id}
            d={path.d}
            stroke="url(#path-gradient-signup)"
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-green-100 p-6 md:p-8 relative overflow-hidden">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600 text-xs md:text-sm">
              Join and start tracking your intramural journey
            </p>
          </div>
      ...
      {showLoadingOverlay && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="flex flex-col items-center space-y-6">
            ...
            <p className="text-green-700 font-semibold text-xl tracking-wide">Redirecting to Login...</p>
          </div>
        </div>
      )}
    </div>
